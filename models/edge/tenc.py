...
class diffusion():
    def __init__(self, timesteps, beta_start, beta_end, w, beta_sche='exp'):
...
class Tenc(nn.Module):
    ...
    def cacu_h(self, states, len_states, p):
        #hidden
        inputs_emb = self.node_embeddings(states)
    ...
    def predict(self, states, len_states, diff):
        ...
        test_item_emb = self.node_embeddings.weight
...
if __name__ == '__main__':
    ...
    # 初始化模型
    model = Tenc(hidden_size, node_num, seq_len, dropout=0.1, diffuser_type='mlp1', device=device)
    model.to(device)
    ...
    beta_end = 0.02
    w = 0.0
    diff = diffusion(timesteps, beta_start, beta_end, w)
    
    # 生成随机输入数据
    random_states = torch.randint(0, node_num, (batch_size, seq_len)).to(device)
    random_len_states = torch.randint(1, seq_len+1, (batch_size,)).to(device)
    
    # 确保长度有效
    for i in range(batch_size):
        if random_len_states[i] < 1:
            random_len_states[i] = 1
    
    print("输入形状:")
    print(f"states: {random_states.shape}")
    print(f"len_states: {random_len_states.shape}")
    
    # 前向传播获取隐藏状态
    with torch.no_grad():
        h = model.cacu_h(random_states, random_len_states, p=0.0)
        print(f"隐藏状态 h 形状: {h.shape}")
        
        # 使用扩散模型生成输出
        x = diff.sample(model.forward, model.forward_uncon, h)
        print(f"扩散模型输出 x 形状: {x.shape}")
        
        # 计算得分
        scores = torch.matmul(x, model.node_embeddings.weight.transpose(0, 1))
        print(f"最终得分形状: {scores.shape}")
        
        # 获取预测的top-k节点
        topk = 10
        _, top_indices = torch.topk(scores, k=topk, dim=1)
        print(f"Top-{topk}预测结果形状: {top_indices.shape}")
        
        print("\n测试样例的Top-5预测结果:")
        for i in range(min(5, batch_size)):
            print(f"样例 {i}: {top_indices[i, :5].cpu().numpy()}")
