...
from .utils.DataLoader import Data, NodeSeqData
...
def evaluate_model_node_classification(model_name: str, 
                                       model: nn.Module, 
                                       neighbor_sampler: NeighborSampler, 
                                        evaluate_idx_data_loader: DataLoader,
                                        evaluate_data: NodeSeqData, 
                                        loss_func: nn.Module, 
                                        node_raw_features: torch.Tensor,
                                        train_dgnn: bool = False, 
                                        use_feature: str = 'no'
                                   ):
    """
    评估节点分类模型在验证数据上的性能
    :param model_name: str, 模型名称
    :param model: nn.Module, 要评估的模型
    :param neighbor_sampler: NeighborSampler, 邻居采样器
    :param evaluate_idx_data_loader: DataLoader, 评估索引数据加载器
    :param evaluate_data: NodeSeqData, 要评估的数据
    :param loss_func: nn.Module, 损失函数
    :param train_dgnn: bool, 是否使用动态图神经网络作为条件
    :return: evaluate_losses, evaluate_metrics
    """
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer']:
        # 评估阶段使用所有图信息
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            evaluate_data_indices = evaluate_data_indices.numpy()
            batch_src_node_degrees_eval, batch_src_node_interact_times_eval = evaluate_data[evaluate_data_indices]
            
            # 使用因果推断方式评估
            batch_loss = 0
            seq_len = batch_src_node_degrees_eval.shape[1]
            
            t = seq_len - 1
            # 使用前t个时间步预测第t+1个时间步
            input_seq = torch.tensor(batch_src_node_degrees_eval[:, :t].reshape(-1,t,1), device=next(model.parameters()).device, dtype=torch.float32)
            target_degree = torch.tensor(batch_src_node_degrees_eval[:, t].reshape(-1,1,1), device=next(model.parameters()).device, dtype=torch.float32)
            input_times = batch_src_node_interact_times_eval[:, :t]
            
            if train_dgnn:
                # 计算节点嵌入
                batch_src_node_embeddings = model[0].compute_src_node_temporal_embeddings(evaluate_data_indices, input_times[:,-1])
                
                # 前向传播，获取quantile权重和重构的degree值
                log_quantile_weights, reconstructed_degrees = model[1](x_degree=input_seq, 
                                                                        x_condition=batch_src_node_embeddings,
                                                                        return_quantile=True)
            else:
                if use_feature == 'no':
                    node_feature = None
                else:
                    node_feature = torch.tensor(node_raw_features[evaluate_data_indices], 
            ...
            target_quantile = model[1].degree_converter.transform(target_degree)
            target_quantile_weights = torch.exp(target_quantile).squeeze(1)
            
            # 使用组合损失函数
            loss, degree_loss, quantile_loss, histogram_loss = loss_func(
                reconstructed_degrees, target_degree, log_quantile_weights, target_quantile_weights
            )
            
            batch_loss += loss
            mmd_distance = compute_mmd_rbf(reconstructed_degrees.cpu().numpy(), target_degree.cpu().numpy())
            # 存储预测和目标值，用于计算MMD距离
            all_predicted_degrees.append(reconstructed_degrees.cpu())
            all_target_degrees.append(target_degree.cpu())
...
def evaluate_model_node_regression(model_name: str, 
                                   model: nn.Module, 
                                   neighbor_sampler: NeighborSampler, 
                                   evaluate_idx_data_loader: DataLoader,
                                   evaluate_data: NodeSeqData, 
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            input_edge_ids, pred_edge_ids  = evaluate_data[evaluate_data_indices]
            
            # 转换为张量并移动到设备
            batch_src_node_feature = torch.tensor(batch_src_node_feature, dtype=torch.float32).to(next(model.parameters()).device)
            batch_src_node_interact_times = torch.tensor(batch_src_node_interact_times, dtype=torch.float32).to(next(model.parameters()).device)
            batch_src_node_pred_feature = torch.tensor(batch_src_node_pred_feature, dtype=torch.float32).to(next(model.parameters()).device)
            batch_src_node_pred_degree = torch.tensor(batch_src_node_pred_degree, dtype=torch.float32).to(next(model.parameters()).device)
            batch_src_node_pred_interact_times = torch.tensor(batch_src_node_pred_interact_times, dtype=torch.float32).to(next(model.parameters()).device)
            
            # 使用与训练相同的模型输入参数
            pred_degree = model[0](batch_src_node_feature, batch_src_node_interact_times,
                                   batch_src_node_pred_feature, batch_src_node_pred_interact_times)
            
            # 计算损失
            loss = loss_func(pred_degree, batch_src_node_pred_degree)
            
            batch_loss += loss
            # 存储预测和目标值，用于计算MMD距离
            all_predicted_degrees.append(pred_degree.cpu())
            all_target_degrees.append(batch_src_node_pred_degree.cpu())
    ...
    # 计算所有批次的损失和MMD距离
    evaluate_losses_allbatches = loss_func(torch.cat(all_predicted_degrees), torch.cat(all_target_degrees))
    ...
    all_predict_history_degrees = torch.tensor(np.expm1(evaluate_data.src_node_degrees[:, 
                                                        :evaluate_data.input_len]))
    if quantile_mapping:
    ...
    mae_mse_metrics = calculate_mae_mse(all_predicted_degrees, all_target_degrees)
    
    # 保存目标度数据到文件
    torch.save(all_target_degrees, f"{mode}_degrees.pt")
    ...
    histogram = HistogramMatchingLoss(num_bins = 100, 
                                      ...
                                      use_mmd=True,
                                      kernel_type='rbf')
    histogram_loss = histogram.forward(all_predicted_degrees, all_target_degrees)
    
    loss = {**mae_mse_metrics, **histogram_loss}
    loss = {
        k:v.item() for k,v in loss.items()
    }
    evaluate_metrics_all = {
        'total_loss_allbatches': evaluate_losses_allbatches.item(),
        **loss
    }
...
def evaluate_model_graph_generation(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluate_data: Data, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000):
    """
    评估图生成模型在验证数据上的性能
