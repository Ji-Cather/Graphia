...
def format_dataset_info_to_latex():
    # 读取CSV文件
    df = pd.read_csv('LLMGGen/reports/concat/dataset_info.csv')
    ...
    # 保存到文件
    with open('LLMGGen/reports/concat/dataset_info.tex', 'w') as f:
        f.write(latex_table)
    
    print("✅ Dataset info LaTeX table saved to: LLMGGen/reports/concat/dataset_info.tex")
