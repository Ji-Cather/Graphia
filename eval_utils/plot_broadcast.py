...
import numpy as np
import os
from LLMGGen.utils.bwr_ctdg import BWRCTDGALLDataset, Dataset_Template


def create_broadcast_comparison_plot(save_path='LLMGGen/reports/figures'):
    """
    ...
    # 读取四组数据
    # grpo_weibo_daily数据集
    daily_broadcast = pd.read_csv('LLMGGen/results/grpo_weibo_daily_rl-all-domain_100epoch/weibo_daily/test/inference/edge_ggen_broadcast.csv')
    daily_normal = pd.read_csv('LLMGGen/results/grpo_weibo_daily_rl-all-domain_100epoch/weibo_daily/test/inference/edge_ggen.csv')
    
    # grpo_weibo_tech数据集
    tech_broadcast = pd.read_csv('LLMGGen/results/grpo_weibo_tech_rl-all-domain_100epoch/weibo_tech/test/inference/edge_ggen_broadcast.csv')
    tech_normal = pd.read_csv('LLMGGen/results/grpo_weibo_tech_rl-all-domain_100epoch/weibo_tech/test/inference/edge_ggen.csv')
...
def create_normalized_comparison(save_path='LLMGGen/reports/figures'):
    """
    创建标准化的对比图（百分比形式）
...
def create_broadcast_comparison_with_labels(save_path='LLMGGen/reports/figures'):
    """
    ...
    tech_data = BWRCTDGALLDataset(
        root="LLMGGen/data/weibo_tech"
    )
    daily_data = BWRCTDGALLDataset(
        root="LLMGGen/data/weibo_daily"
    )
    type_color_map = {
        "comment": "#F79927",
        "repost": "#2F5763"
...
if __name__ == "__main__":
    try:
        ...
        print(f"文件未找到: {e}")
        print("请确认以下文件路径是否正确:")
        print("1. LLMGGen/results/grpo_weibo_daily_rl-all-domain_100epoch/weibo_daily/test/inference/edge_ggen_broadcast.csv")
        print("2. LLMGGen/results/grpo_weibo_daily_rl-all-domain_100epoch/weibo_daily/test/inference/edge_ggen.csv")
        print("3. LLMGGen/results/grpo_weibo_tech_rl-all-domain_100epoch/weibo_tech/test/inference/edge_ggen_broadcast.csv")
        print("4. LLMGGen/results/grpo_weibo_tech_rl-all-domain_100epoch/weibo_tech/test/inference/edge_ggen.csv")
