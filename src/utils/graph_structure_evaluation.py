...
class Descriptor():
    ...
    def evaluate_from_degree_array(self, generated_dataset_degree, reference_dataset_degree):
        sample_pred = []
        sample_ref = []
        
        if len(generated_dataset_degree.shape) == 1:
            generated_dataset_degree = generated_dataset_degree.reshape(1, -1)
        if len(reference_dataset_degree.shape) == 1:
            reference_dataset_degree = reference_dataset_degree.reshape(1, -1)
        
        max_degree = 100
        hist = 100
        
        def bin_degrees(degree_counts, max_degree, hist):
            bin_width = max_degree / hist
            binned_counts = [0] * hist
            for degree, count in degree_counts.items():
                if degree < max_degree:
                    bin_idx = min(int(degree / bin_width), hist - 1)
                    binned_counts[bin_idx] += count
                else:
                    # 超过最大度数的都放在最后一个箱中
                    binned_counts[-1] += count
            return binned_counts[1:] # 去除degree == 0的箱
        
        for g_degree in generated_dataset_degree:
            counts = Counter(g_degree)
            sample_pred.append(bin_degrees(counts, max_degree, hist))
            
        for g_degree in reference_dataset_degree:
            counts = Counter(g_degree)
            sample_ref.append(bin_degrees(counts, max_degree, hist))

        sample_pred = np.array(sample_pred)/ np.sum(sample_pred)
        sample_ref = np.array(sample_ref)/ np.sum(sample_ref)
...
if __name__ == "__main__":
    # 设置一个 Degree 的 descriptor
    descriptor = Degree(is_parallel = False)

    # 随机生成一个 nx.Graph
    random_graph = nx.erdos_renyi_graph(n=10, p=0.1)
    degree = nx.degree_histogram(random_graph)

    metric = descriptor.evaluate_from_degree_array(np.array([degree for node,degree in random_graph.degree()]), 
                                                   np.array([degree for node,degree in random_graph.degree()]))
    print("Metric:", metric)