...
class DyGFormer(nn.Module):
    ...
    def compute_src_node_temporal_embeddings(self, src_node_ids: np.ndarray, src_node_interact_times: np.ndarray):
        """
        只计算源节点的时序嵌入
        :param src_node_ids: ndarray, shape (batch_size, )
        :param src_node_interact_times: ndarray, shape (batch_size, )
        :return: Tensor, shape (batch_size, node_feat_dim)
        """
        # 获取源节点的一阶邻居
        src_nodes_neighbor_ids_list, src_nodes_edge_ids_list, src_nodes_neighbor_times_list = \
            self.neighbor_sampler.get_all_first_hop_neighbors(node_ids=src_node_ids, node_interact_times=src_node_interact_times)

        
        # 对源节点的一阶邻居序列进行填充
        src_padded_nodes_neighbor_ids, src_padded_nodes_edge_ids, src_padded_nodes_neighbor_times = \
            self.pad_sequences(node_ids=src_node_ids, node_interact_times=src_node_interact_times, 
        ...
        # 计算邻居共现特征
        src_padded_nodes_neighbor_co_occurrence_features, _ = \
            self.neighbor_co_occurrence_encoder(src_padded_nodes_neighbor_ids=src_padded_nodes_neighbor_ids,
                                             dst_padded_nodes_neighbor_ids=src_padded_nodes_neighbor_ids)

        # 获取序列的特征
        src_padded_nodes_neighbor_node_raw_features, src_padded_nodes_edge_raw_features, src_padded_nodes_neighbor_time_features = \
            self.get_features(node_interact_times=src_node_interact_times, 
        ...
        # 将所有特征堆叠在一起
        patches_data = torch.stack(patches_data, dim=2)
        patches_data = patches_data.reshape(batch_size, src_num_patches, self.num_channels * self.channel_embedding_dim)
        ...
        # 平均所有patches
        patches_data = torch.mean(patches_data, dim=1)

        # 生成最终的节点嵌入
        src_node_embeddings = self.output_layer(patches_data)
