...
from .evaluate_models_utils import evaluate_model_node_regression
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        # degree_regressor = Informer(
        #                         enc_in=train_node_seq_data.src_node_feature_dim,
        #                         dec_in=train_node_seq_data.src_node_feature_dim,
        #                         c_out=train_node_seq_data.pred_len,  # 输出维度（DAU为单变量）
        #                         seq_len=train_node_seq_data.input_len,
        #                         label_len=train_node_seq_data.input_len,
        #                         out_len=train_node_seq_data.pred_len,
        #                         factor=4,
        #                         d_model=64,
        #                         n_heads=4,
        #                         e_layers=2,
        ...
        degree_regressor = InformerTranverse(
                                bwr = train_node_seq_data.bwr,
                                node_feature_dim = train_node_seq_data.node_feature.shape[1]+2,
                                input_len = train_node_seq_data.input_window_len,
        ...
        model = nn.Sequential(degree_regressor)
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, node_seq_idx in enumerate(train_idx_data_loader_tqdm):
                ...
                input_edge_ids, pred_edge_ids = train_node_seq_data[node_seq_idx]
                ...
                batch_src_node_pred_degree = torch.tensor(batch_src_node_pred_degree, dtype=torch.float32).to(args.device)
                ...
                loss = criterion(pred_degree, batch_src_node_pred_degree)
                ...
                train_pred_degrees_all.append(pred_degree)
                train_target_degrees_all.append(batch_src_node_pred_degree)
            
            train_pred_degrees_all = torch.cat(train_pred_degrees_all, dim=0)
            train_target_degrees_all = torch.cat(train_target_degrees_all, dim=0)
            batch_loss = criterion(train_pred_degrees_all, train_target_degrees_all)
            logger.info(f'Epoch: {epoch + 1}, train loss: {batch_loss.item()}')
            

            eval_losses, eval_metrics, eval_metrics_all = evaluate_model_node_regression(model_name=args.model_name,
                                                                        model=model,
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                test_losses, test_metrics, test_metrics_all = evaluate_model_node_regression(model_name=args.model_name,
                                                                          model=model,
                                                                          neighbor_sampler=full_neighbor_sampler,
