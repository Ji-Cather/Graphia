...
def evaluate_hubs(gt_edge_matrix, pred_edge_matrix, k=20):
    ...
    返回:
    dict, 包含precision、f1、auc和ndcg@k的评估结果
    """
...
def evaluate_all_sources(gt_edge_matrix, pred_edge_matrix, 
                         gt_graph, pred_graph, 
    ...
    for i in test_src_indices:
        src_results.append(evaluate_src(gt_edge_matrix[i,:], pred_edge_matrix[i,:], k = 10))
    
    
    jl_result = jl_all_graph(gt_graph, pred_graph)
    abs_result = eval_graph_metric(gt_edge_matrix, pred_edge_matrix)
    ...
    src_agg.update({f"{k}_hub": v for k, v in pred_result.items()})

    edge_overlap = np.sum((gt_edge_matrix > 0) & (pred_edge_matrix > 0)) / np.sum(gt_edge_matrix > 0)
    src_agg['edge_overlap'] = edge_overlap
    src_agg.update(jl_result)
    src_agg.update(abs_result)
...
if __name__ == "__main__":  
    ...
    # 获取test_edge_matrix中行和不为0的索引
    test_src_indices = np.where(test_edge_matrix.sum(axis=1) > 0)[0][:10]
    ...
    for baseline_name, baseline_data in [('tigger', tigger_data), ('dggen', dggen_data)]:
        ...
        baseline_results = evaluate_all_sources(test_edge_matrix,baseline_edge_matrix, 
                                                test_data, baseline_data, 
                                                test_src_indices)
        baseline_results["model"] = baseline_name
