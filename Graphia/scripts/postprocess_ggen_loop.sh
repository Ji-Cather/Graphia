...
set_tf_wofilter_paths() {
    query_file_name="query_examples.csv"
    query_save_path="${llm_save_root}/${data_name}/test/teacher_forcing/${query_file_name}"
    query_result_path="${llm_save_root}/${data_name}/test/teacher_forcing/query_ggen_wofilter.csv"
    graph_report_path="${report_save_root}/${data_name}/test/teacher_forcing/graph_matrix_wofilter.csv"
}
...
case "$mode" in
    "process_tf_wofilter")
        echo "Mode: Process tdgg wofilter query"
        set_tf_wofilter_paths
    ...
    "eval_inf")
        ...
        ;;
    "help"|"")
        echo "Usage: mode={process_tf|process_tf_wofilter|process_inf|eval_inf}"
        echo "Current mode: '$mode'"
        exit 1
        ;;
    *)
        echo "Unknown mode: $mode"
        echo "Available: process_tf_wofilter, process_tf, process_inf, eval_inf"
        exit 1
        ;;
esac
