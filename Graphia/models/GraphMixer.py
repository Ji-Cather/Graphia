...
class GraphMixer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_neighbors: int, num_layers: int = 2, token_dim_expansion_factor: float = 0.5,
        ...
        :param num_neighbors: int, number of tokens
        ...
        self.mlp_mixers = nn.ModuleList([
            MLPMixer(num_neighbors=self.num_neighbors, num_channels=self.num_channels,
...
class MLPMixer(nn.Module):
    ...
    def __init__(self, num_neighbors: int, num_channels: int, token_dim_expansion_factor: float = 0.5,
        ...
        self.token_norm = nn.LayerNorm(num_neighbors)
        self.token_feedforward = FeedForwardNet(input_dim=num_neighbors, dim_expansion_factor=token_dim_expansion_factor,
                                                dropout=dropout)
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        :param input_tensor: Tensor, shape (batch_size, num_neighbors, num_channels)
        :return:
        """
        # mix tokens
        # Tensor, shape (batch_size, num_channels, num_neighbors)
        hidden_tensor = self.token_norm(input_tensor.permute(0, 2, 1))
        # Tensor, shape (batch_size, num_neighbors, num_channels)
        hidden_tensor = self.token_feedforward(hidden_tensor).permute(0, 2, 1)
        # Tensor, shape (batch_size, num_neighbors, num_channels), residual connection
