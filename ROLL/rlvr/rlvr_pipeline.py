...
class RLVRPipeline(BasePipeline):
    ...
    def __init__(self, pipeline_config: RLVRConfig):
        ...
        for i, domain in enumerate(domain_list):
            ...
            # debug
            # import deepspeed.model_implementations.transformers.ds_transformer
            # print(dir(deepspeed.model_implementations.transformers.ds_transformer))
            ray.get(
        ...
        for key, cluster in self.rewards.items():
            refs.extend(cluster.initialize(pipeline_config=self.pipeline_config, blocking=True))
    ...
    def val(self):
        ...
        batch = generate_output
        val_correct_mean = (batch.batch["scores"]).detach().float().mean().item()
        val_metrics_mgr.add_metric("val_correct/all/mean", val_correct_mean)
        logger.info(json.dumps({"val_correct/all/mean": val_correct_mean}, ensure_ascii=False))

        val_correct_level_mean = ((batch.batch["scores"]> 0).float()).detach().float().mean().item()
        val_metrics_mgr.add_metric("val_correct/all/level_mean", val_correct_mean)
        logger.info(json.dumps({"val_correct/all/level_mean": val_correct_mean}, ensure_ascii=False))
        ...
        for group_key, group_batch in grouped_batch.items():
            score_mean = group_batch.batch["scores"].mean().item()
            print(f"{group_key}:  {score_mean}")
            level_mean = ((group_batch.batch["scores"]> 0).float()).detach().float().mean().item()
            val_metrics_mgr.add_domain_metrics(
                "val_correct", {f"{group_key}/mean": (group_batch.batch["scores"]).detach().float().mean().item()}
            )
            val_metrics_mgr.add_domain_metrics(
                "val_correct", {f"{group_key}/level_mean": level_mean}
            )
