...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--data_name', type=str, help='dataset to be used', default='Cite_HepTH')
    ...
    parser.add_argument("--bwr", type=int, default=2048, help="Batch width for the graph data loader")
    parser.add_argument('--time_window', type=int, default=24*60*60, help='时间窗口大小')
    parser.add_argument("--cm_order", action="store_true", help="Whether to reorder the nodes with cm_order")
...
def get_node_classification_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--load_best_configs', action='store_true', default=True, help='whether to load the best configurations')
    parser.add_argument("--train_dgnn", action="store_true", help="Whether to train the DGNN model")
    try:
        args = parser.parse_args()
        args.device = f'cuda:{args.gpu}' if torch.cuda.is_available() and args.gpu >= 0 else 'cpu'
    except:
...
def get_node_regression_args(is_evaluation: bool = False):
    """
    get the args for the node regression task
    :param is_evaluation: boolean, whether in the evaluation process
    :return:
    """
    # arguments
    parser = argparse.ArgumentParser('Interface for the Node Regression task')
    
    parser.add_argument('--model_name', type=str, default='Informer', help='name of the model, note that EdgeBank is only applicable for evaluation') # TGAT,CAWN 
    parser.add_argument('--batch_size', type=int, default=256, help='batch size')
    parser.add_argument('--gpu', type=int, default=0, help='number of gpu to use')
    parser.add_argument('--val_ratio', type=float, default=0.15, help='ratio of validation set')
    parser.add_argument('--test_ratio', type=float, default=0.15, help='ratio of test set')
